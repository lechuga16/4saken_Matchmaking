...
void PerformTests() {
    ...
    OS os = System2_GetOS();
    assertValueNotEquals(view_as<int>(OS_UNKNOWN), view_as<int>(os));
    PrintToServer("INFO: The OS is: %s", (os == OS_WINDOWS) ? "Windows" : ((os == OS_UNIX) ? "Linux" : "MAC"));
...
void PerformLegacyTests() {
    ...
    assertValueEquals(view_as<int>(CMD_SUCCESS), view_as<int>(System2_RunCommand(output, sizeof(output), "echo thisIsANonThreadedTestCommand")));
    TrimString(output);
    assertStringEquals("thisIsANonThreadedTestCommand", output);
...
void LegacyCommandCallback(const char[] output, const int size, CMDReturn status, any data, const char[] command) {
    ...
    if (data == TEST_AGENT) {
        ...
        assertValueEquals(view_as<int>(CMD_SUCCESS), view_as<int>(status));
        ...
        if (strlen(longPage) < 4238) {
            assertValueEquals(view_as<int>(CMD_PROGRESS), view_as<int>(status));
        } else {
            finishedCallbacks++;
