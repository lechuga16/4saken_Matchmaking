...
{
	char sQuery[300];
	g_ForsakenDB.Format(sQuery, sizeof(sQuery), "INSERT INTO `%s` (map, survivor, infected, chip, overkill) VALUES ('%s', '%s', '%s', '%d', '%d');", sTFormat2[iTable], sMapName, sSteamID, sSteamID2, damage, view_as<int>(isOverkill));
	g_ForsakenDB.Query(UpdateTFormat2, sQuery, iTable);
}

public void UpdateTFormat2(Database db, DBResultSet results, const char[] error, any iTable)
{
	if (results == null)
		ThrowError("Error UpdateTFormat2 %s: %s", sTFormat2[iTable], error);
}