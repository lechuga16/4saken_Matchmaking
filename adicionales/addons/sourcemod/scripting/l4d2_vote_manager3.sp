...
public Action CustomVote(int client, int args)
{
	...
	if ((ClientHasAccess(client, "cooldown_immunity") || iNextVote[client] <= flEngineTime) && VoteStatus == VOTE_NONE && args >= 2 && ClientHasAccess(client, "custom"))
	{
		...
		VoteManagerNotify(client, "%t %t", "Tag", "Custom Vote", client, arg1, sOption, sCmd);
...
public Action VoteStart(int client, const char[] command, int argc)
{
	...
	if (argc >= 1)
	{
		...
		if ((ClientHasAccess(client, "cooldown_immunity") || iNextVote[client] <= flEngineTime) && VoteStatus == VOTE_NONE || IsNewBuiltinVoteAllowed())
		{
			...
			if (ClientHasAccess(client, sIssue))
			{
				...
				{
					CReplyToCommand(client, "%T", "Use sm_customvote", client);
					return Plugin_Handled;
				}
				...
				else
				{
					if (argc == 2)
					{
						...
						VoteManagerNotify(client, "%t %t", "Tag", "Vote Called 2 Arguments", sCaller, sIssue, sOption);
					...
					else
					{
						...
						VoteManagerNotify(client, "%t %t", "Tag", "Vote Called", sCaller, sIssue);
			...
			else
			{
				...
				VoteManagerNotify(client, "%t %t", "Tag", "No Access", sCaller, sIssue);
				VoteLogAction(client, -1, "'%L' callvote denied (reason 'no access')", client);
				ClearVoteStrings();
				return Plugin_Handled;
			}
		}
		else if (VoteStatus == VOTE_POLLING || !IsNewBuiltinVoteAllowed())
		{
			CPrintToChat(client, "%t %T", "Tag", "Conflict", LANG_SERVER);
			VoteLogAction(client, -1, "'%L' callvote denied (reason 'vote already called')", client);
			ClearVoteStrings();
			return Plugin_Handled;
		}
		else if (iNextVote[client] > flEngineTime || iNextVote[client] > CheckBuiltinVoteDelay())
		{
			CPrintToChat(client, "%t %T", "Tag", "Wait", LANG_SERVER, RoundToNearest(iNextVote[client] - flEngineTime));
...
public Action Command_VoteVeto(int client, int args)
{
	if (VoteStatus == VOTE_POLLING && ClientHasAccess(client, "veto"))
	{
		...
		else
		{
			...
			CReplyToCommand(client, "%t %T", "Tag", "Cant Veto", LANG_SERVER, client);
			VoteLogAction(client, -1, "'%L' sm_veto ('not enough undecided players')", client);
			return Plugin_Handled;
		}
		LogVoteManager("%T", "Vetoed", LANG_SERVER, client);
		CReplyToCommand(client, "%t %T", "Tag", "Vetoed", LANG_SERVER, client);
	...
	{
		CReplyToCommand(client, "%t %T", "Tag", "No Vote", LANG_SERVER);
...
public Action Command_VotePassvote(int client, int args)
{
	if (VoteStatus == VOTE_POLLING && ClientHasAccess(client, "pass"))
	{
		...
		{
			LogVoteManager("%T", "Cant VetoPass", LANG_SERVER, client);
			CReplyToCommand(client, "%t %T", "Tag", "Cant Pass", LANG_SERVER, client);
			VoteLogAction(client, -1, "'%L' sm_veto ('not enough undecided players')", client);
			return Plugin_Handled;
		}
		LogVoteManager("%T", "Passed", LANG_SERVER, client);
		CReplyToCommand(client, "%t %T", "Tag", "Passed", LANG_SERVER, client);
...
stock Action ClientCanKick(int client, const char[] userid)
{
	...
	if (0 >= target || target > MaxClients || !IsClientInGame(target))
	{
		...
		VoteManagerNotify(client, "%t %t", "Tag", "Invalid Kick Userid", client, userid);
	...
	if (GetConVarBool(hTankImmunity) && IsPlayerAlive(target) && cTeam == 3 && GetEntProp(target, Prop_Send, "m_zombieClass") == 8)
	{
		...
		VoteManagerNotify(client, "%t %t", "Tag", "Tank Immune Response", client, target);
	...
	if (cTeam == 1)
	{
		...
		VoteManagerNotify(client, "%t %t", "Tag", "Spectator Response", client, target);
	...
	if (GetConVarBool(hRespectImmunity) && id != INVALID_ADMIN_ID && targetid != INVALID_ADMIN_ID)    // both targets need to be admin.
	{
		if (!CanAdminTarget(id, targetid))
		{
			...
			VoteManagerNotify(client, "%t %t", "Tag", "Kick Vote Call Failed", client, target);
	...
	if (CheckCommandAccess(target, "kick_immunity", 0, true) && !CheckCommandAccess(client, "kick_immunity", 0, true))
	{
		...
		VoteManagerNotify(client, "%t %t", "Tag", "Kick Immunity", client, target);
