...
public SMCResult MissionParser_NewSection(SMCParser smc, const char[] name, bool opt_quotes)
{
	switch (g_MissionParser_State)
	{
		...
		case MPS_UNKNOWN:
		{    // Traverse through unknown structures
		...
		case MPS_GAMEMODE:
		{
			...
			for (int iMap = 1; iMap <= g_hIntMap_Index.Length; iMap++)
			{
				...
				if (index < 0)
				{
					...
					BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
					LogToFile(sPath, "Mission %s contains invalid \"%s\" section", g_MissionParser_MissionName, gamemodeName);
				...
				if (!IsMapValid(mapFile))
				{
					...
					BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
					LogToFile(sPath, "Mission %s contains invalid map: \"%s\", gamemode: \"%s\"", g_MissionParser_MissionName, mapFile, gamemodeName);
					return SMCParse_HaltFail;
				}
				numOfValidMaps++;
			}
			...
			if (numOfValidMaps < 1)
			{
				...
				LMM_GamemodeToString(g_MissionParser_CurGameMode, gamemodeName, sizeof(gamemodeName));
				BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
				LogToFile(sPath, "Mission %s does not contain any valid map in gamemode: \"%s\"", g_MissionParser_MissionName, gamemodeName);
				return SMCParse_Continue;
			}
...
void CacheMissions()
{
	...
	{
		BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
		LogToFile(sPath, "[SM] Plugin is not running! Could not locate mission folder");
...
void ParseMissions()
{
	...
	{
		BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
		LogToFile(sPath, "The \"missions.cache\" folder was not found!");
	}
	else {
		// Create the parser
		...
		while (dirList.GetNext(missionFileName, PLATFORM_MAX_PATH, fileType))
		{
			if (fileType == FileType_File)
			{
				...
				if (err != SMCError_Okay)
				{
					...
					BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
					LogToFile(sPath, "An error occured while parsing %s, code:%d", missionCache, err);
				}
			}
		}
...
void ParseLocalization(LMM_GAMEMODE gamemode)
{
	...
	{
		BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
		LogToFile(sPath, "Mission name localization file %s does not exist!", missonsPhrasesEnglish);
		// TO-DO:
	}
	...
	{
		BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
		LogToFile(sPath, "Map name localization file %s does not exist!", mapsPhrasesEnglish);
		// TO-DO:
	}
	...
	{
		BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
		LogToFile(sPath, "An error occured while parsing missions.phrases.txt(English), code:%d", err);
	}
	...
	{
		BuildPath(Path_SM, sPath, sizeof(sPath), "logs/%s.log", LOGNAME);
		LogToFile(sPath, "An error occured while parsing maps.phrases.txt(English), code:%d", err);
	}
}
